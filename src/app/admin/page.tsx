...
export default function AdminPage() {
  ...
  async function handleGenerateAssignments() {
    ...
    const confirmed = confirm(
      `Generate assignments for ${passengers.filter((p) => p.attending).length} passengers across ${drivers.filter((d) => d.available).length} cars? This will overwrite any existing assignments.`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      toast.success(
        `✅ Assigned ${data.totalPassengersAssigned} passengers to ${data.assignmentsCreated} cars!`
      );
      await loadData();
    } catch {
  ...
  async function handleLockTrip() {
    if (!trip) return;
    const confirmed = confirm("Lock assignments? Passengers and drivers will be notified. No more changes can be made.");
    if (!confirmed) return;
    ...
    try {
      ...
      toast.success("🔒 Trip locked. Share the assignments!");
      await loadData();
    } catch {
    ...
    <div className="page-container">
      ...
      <div className="page-header">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
              ...
              {trip && (
                <p className="text-xs text-gray-500">
                  ...
                  <span className={`font-medium ${
                    trip.status === "LOCKED" ? "text-blue-600"
          ...
          <button onClick={loadData} className="text-gray-400 hover:text-gray-600 p-2" title="Refresh">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className={`card text-center ${!capacityOk ? "bg-red-50 border-red-100" : ""}`}>
                <p className={`text-2xl font-bold ${capacityOk ? "text-green-500" : "text-red-500"}`}>
                  {totalSeats}
                </p>
                <p className="text-xs text-gray-500 mt-0.5">Total Seats</p>
                {!capacityOk && (
                  <p className="text-xs text-red-500 mt-1">
                    ⚠️ {attendingCount - totalSeats} over capacity
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="card">
                ...
                <div className="w-full bg-gray-100 rounded-full h-2">
                  <div
                    ...
                    style={{ width: `${(assignedCount / attendingCount) * 100}%` }}
                  />
                </div>
                {assignedCount === attendingCount && (
            ...
            <div className="space-y-2">
              <button
                ...
                disabled={generating || attendingCount === 0 || availableDriverCount === 0 || trip?.status === "LOCKED"}
            ...
            <div className="flex bg-gray-100 rounded-xl p-1">
              <button
                ...
                className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${
                  activeTab === "passengers" ? "bg-white shadow-sm text-gray-900" : "text-gray-500"
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${
                  activeTab === "drivers" ? "bg-white shadow-sm text-gray-900" : "text-gray-500"
                }`}
              >
            ...
            {activeTab === "passengers" && (
              <div className="space-y-2">
                ...
                {passengers.map((p) => (
                  <div key={p.id} className={`card flex items-start gap-3 ${!p.attending ? "opacity-50" : ""}`}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                      p.attending ? "bg-orange-100 text-orange-600" : "bg-gray-100 text-gray-400"
                    }`}>
                      {p.name[0]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {p.attending && p.assignmentId && <span className="badge badge-green">Assigned</span>}
                        {p.attending && !p.assignmentId && <span className="badge badge-orange">Unassigned</span>}
                      </div>
                    ...
                    <a href={`tel:${p.phone}`} className="text-gray-400 hover:text-gray-600 flex-shrink-0 p-1">
                  ...
                  <div key={d.id} className={`card flex items-start gap-3 ${!d.available ? "opacity-50" : ""}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        🚗 {d.carType || "Car not specified"} · {d.seats} seats
                      </p>
                      {d.passengerCount > 0 && (
                        <p className="text-xs text-green-600 mt-0.5">
                          👥 {d.passengerCount}/{d.seats} passengers assigned
                        </p>
                      )}
                    </div>
                    <a href={`tel:${d.phone}`} className="text-gray-400 hover:text-gray-600 flex-shrink-0 p-1">
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                      </svg>
                    </a>
